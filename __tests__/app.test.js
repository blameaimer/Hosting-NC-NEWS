...
describe("/api/articles", () => {
  describe("GET", () => {
    ...
    test("400 for invalid limit input", () => {
      return request(app).get("/api/articles?limit=waffle&&p=1").expect(400);
    });
    ...
    test("404 for invalid page", () => {
      return request(app).get("/api/articles?limit=1&&p=432432").expect(404);
    });
  });
  describe("POST", () => {
    test("this test should have a return of the articles about mitchs", () => {
    ...
    describe("DELETE", () => {
      test("should return an empty response body ", () => {
        return request(app)
          ...
          .then(() => {
            return db.query(
              "SELECT article_id FROM comments WHERE article_id = 2"
            );
          })
          .then(({ rows }) => {
            expect(rows).toEqual([]);
